...
CONTROL_ADJUSTED_R2 = {
    "competition": 0.6158906330925098,
    "scientific_proxy_removed": 0.616607643898979,
}
MINIMUM_MATERIAL_IMPROVEMENT = 0.0005


def candidate_name(alpha: float, l1_ratio: float) -> str:
    alpha_text = f"{alpha:g}".replace(".", "p")
    ratio_text = f"{l1_ratio:g}".replace(".", "p")
    return f"elastic_a{alpha_text}_l1_{ratio_text}"


def candidate_grid() -> list[dict[str, float | str]]:
    return [
        {
            "model": candidate_name(alpha, l1_ratio),
            "alpha": alpha,
            "l1_ratio": l1_ratio,
        }
        for alpha in ALPHAS
...
def make_candidate(alpha: float, l1_ratio: float, seed: int) -> ElasticNet:
    return ElasticNet(
        alpha=alpha,
        l1_ratio=l1_ratio,
        max_iter=20000,
...
def run_elastic_tuning(root: Path, overwrite: bool = False) -> list[Path]:
    ...
    config_path = root / "configs" / "task2_elastic_tuning.toml"
    ...
    for contract_name, source_features in contracts.items():
        ...
        for fold in range(5):
            ...
            print(f"[{contract_name} elastic fold {fold + 1}/5] preparing development split...")
            ...
            fold_best_name = ""
            fold_best_adjusted_r2 = -np.inf
            for item in grid:
                model_name = str(item["model"])
                estimator = make_candidate(float(item["alpha"]), float(item["l1_ratio"]), SEED + fold)
                ...
                if metric["adjusted_r2_raw"] > fold_best_adjusted_r2:
                    fold_best_adjusted_r2 = metric["adjusted_r2_raw"]
                    fold_best_name = model_name
            print(f"  best={fold_best_name}: adjR2={fold_best_adjusted_r2:.4f}")
    ...
    for contract_name, source_features in contracts.items():
        ...
        for item in grid:
            ...
            summary_rows.append({
                ...
                "delta_vs_fixed_elastic": pooled["adjusted_r2_raw"] - CONTROL_ADJUSTED_R2[contract_name],
    ...
    winners["material_improvement"] = (
        winners["delta_vs_fixed_elastic"] >= MINIMUM_MATERIAL_IMPROVEMENT
    )

    output_root = root / "outputs"
    for directory in ("tables", "predictions", "logs", "logs/history"):
        (output_root / directory).mkdir(parents=True, exist_ok=True)
    prefix = "task2_cyclic_elastic_tuning"
    paths = [
    ...
    if not overwrite and any(path.exists() for path in paths):
        raise FileExistsError("任务二 Elastic Net 调参输出已存在；确认重跑时请使用 --overwrite")

    identity = hashlib.sha256(
        f"task2|cyclic_elastic_tuning|{_sha256(data_path)}|{_sha256(split_path)}|{_sha256(config_path)}".encode()
    ).hexdigest()[:8]
    run_id = f"{started_at.strftime('%Y%m%dT%H%M%S%z')}_task2_elastic_tuning_{identity}"
    best_by_contract = {
        row["contract"]: {
            "model": row["model"],
            "alpha": float(row["alpha"]),
            "l1_ratio": float(row["l1_ratio"]),
            "oof_adjusted_r2_raw": float(row["oof_adjusted_r2_raw"]),
            "delta_vs_fixed_elastic": float(row["delta_vs_fixed_elastic"]),
    ...
    manifest = {
        ...
        "command": "python scripts/run_task2_elastic_tuning.py",
        "working_directory": str(root),
        "git": _git_state(root),
        "feature_contracts": {name: len(features) for name, features in contracts.items()},
        "time_encoding": "sin_cos_1440_minutes",
        "grid": {"alpha": list(ALPHAS), "l1_ratio": list(L1_RATIOS), "candidate_count": len(grid)},

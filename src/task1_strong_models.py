...
def run_strong_models(root: Path, overwrite: bool = False) -> list[Path]:
    ...
    config_path = root / "configs" / "task1_strong_models.toml"
    ...
    for fold in range(5):
        ...
        for name, estimator in factories.items():
            ...
            prediction_map = dict(zip(valid_ids, prediction))
            mask = oof["Person_ID"].isin(prediction_map)
            oof.loc[mask, f"pred_{name}"] = oof.loc[mask, "Person_ID"].map(prediction_map)
    ...
    prefix = "task1_non_sleep_strong_models_fixed"
    current_paths = [
    ...
    if not overwrite and any(path.exists() for path in current_paths):
        raise FileExistsError("强模型输出已存在；确认重跑时请使用 --overwrite")

    config_hash, split_hash, data_hash = _sha256(config_path), _sha256(split_path), _sha256(data_path)
    identity = hashlib.sha256(
        f"task1|strong_fixed|{config_hash}|{split_hash}|{data_hash}".encode("utf-8")
    ).hexdigest()[:8]
    run_id = f"{started_at.strftime('%Y%m%dT%H%M%S%z')}_task1_strong_fixed_{identity}"
    manifest = {
        "run_id": run_id,
        "status": "completed",
        "task": "task1",
        "target": TARGET,
        "experiment": "non_sleep_strong_models_fixed",
